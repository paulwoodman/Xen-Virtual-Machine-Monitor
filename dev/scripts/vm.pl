...
#$machinesupdate = "UPDATE `machines` SET `uuid` = '$uuid[$x]', `name` = '$name[$x]', `state` = '$state[$x]', `used` = '$lock[$x]' WHERE `machines`.`uuid` = '$uuid[$x]'";
#$machinesupdate_handle = $connect->prepare($machinesupdate);
#$machinesupdate_handle->execute();
#$machinesupdate = $machinesupdate_handle->disconnect;
...
$lockquery = "SELECT used FROM machines WHERE machines.uuid='$lockuuid'";
